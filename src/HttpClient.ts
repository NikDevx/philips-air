...
export class HttpClient implements AirClient {
  ...
  constructor(host: string, timeout = 5000, key?: Uint8Array) {
    ...
    if (!key) {
      ...
      this.key = new Promise<Uint8Array>((resolve) => {
        resolve(key);
      });
    }
  ...
  private getKey(): Promise<Uint8Array> {
    ...
    this.key = fetch('http://' + this.host + '/di/v1/products/0/security', {
      method: 'PUT',
      ...
      .then((dh) => {
        ...
        return aesDecrypt(dh['key'], s_bytes).slice(0, 16);
      });
    return this.key;
  }
  ...
  async setValues(values: any): Promise<void> { // eslint-disable-line @typescript-eslint/explicit-module-boundary-types
    ...
    await fetch('http://' + this.host + '/di/v1/products/1/air', {
      method: 'PUT',
      body: encrypted,
      timeout: this.timeout
    });
  ...
  private async getData(endpoint: string): Promise<any> {
    let data = '';
    try {
      data = await this.getOnce(endpoint);
    } catch {
      this.getKey();
      data = await this.getOnce(endpoint);
    }
